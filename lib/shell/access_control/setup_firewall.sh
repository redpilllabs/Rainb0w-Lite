...
iptables -t nat -A PREROUTING -i $INTERFACE -p tcp --dport 9443 -j DNAT --to-destination :443
ip6tables -t nat -A PREROUTING -i $INTERFACE -p tcp --dport 9443 -j DNAT --to-destination :443

echo -e "${B_GREEN}>> Allow port 993/tcp for MTProto ${RESET}"
iptables -A INPUT -p tcp --dport 9443 -m conntrack --ctstate NEW -m comment --comment "Allow MTProto" -j ACCEPT
ip6tables -A INPUT -p tcp --dport 9443 -m conntrack --ctstate NEW -m comment --comment "Allow MTProto" -j ACCEPT
...
iptables -A INPUT -p udp --dport 9443 -m conntrack --ctstate NEW -m comment --comment "Allow Hysteria (9443)" -j ACCEPT
ip6tables -A INPUT -p udp --dport 9443 -m conntrack --ctstate NEW -m comment --comment "Allow Hysteria (9443)" -j ACCEPT
